...
public class CostCenterController{
    ...
    protected List<CostCenter> getCostCenters(final HttpServletRequest request, final HttpServletResponse response)
...
//		final List<BusinessPartner> costcenters = new GetCostCenterCommand(config,null).execute();
        ...
        ODataQuery a= new DefaultCostCenterService().getAllCostCenter().select(CostCenter.ALL_FIELDS).toQuery();
        logger.info("hello hello hello!!!" + a.toString());
        
        
        logger.info(a.execute("ErpQueryEndpoint").toString());
